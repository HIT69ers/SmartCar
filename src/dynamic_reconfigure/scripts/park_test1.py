...
if __name__ == "__main__":
    rospy.init_node("dynamic_client_global")
    ...
    while not rospy.is_shutdown():
        ...
        if mic_awake:
            break
        rate.sleep()
    client = dynamic_reconfigure.client.Client("/move_base/global_costmap/inflation_layer", timeout = 30, config_callback = Callback)
    while  not rospy.is_shutdown():
        ...
        if x > 0 and x < 2.8 and y > -1.5 and y < 0.15 and parking :
            ...
            rospy.logwarn("____________全局膨胀半径设置完啦__________________")
