...
from my_detect_model import myPredict, myPredict_single, myLoadModel
...
class Detect:
    ...
    def __init__(self, config, class_list, test_mode=0):
        ...
        if test_mode == 0:
            self.model, self.imgsz, self.pt = myLoadModel(
                weights=f'{folder_path}/my_model/' + config['model_file'])  # 获取模型
    ...
    def reference_2(self):
        ...
        results_bs = myPredict_single(model=self.model, im0=self.single_img, imgsz=self.imgsz, pt=self.pt,
                                      stride=32)  # 将单张照片推入网络

        if len(results_bs) > 0:  # 这部分也是暂时测试代码
            results = results_bs[0]  # result x1, y1, x2, y2, con, cls #外层套了一个列表[]，因此要取0
            # 0708
            rospy.set_param("result_cam_0", results[0, 0].item())
            rospy.set_param("result_cam_1", results[0, 1].item())
            rospy.set_param("result_cam_2", results[0, 2].item())
            rospy.set_param("result_cam_3", results[0, 3].item())
        else:
            results = None
