...
class Rec:  # 我的识别类
    ...
    def judge_room(self, position_x, position_y):
        ...
        判断小车坐标是否在某个房间中
        """
        for room_name, area_p in self.room:
    ...
    def main(self, config, detect):
        ...
        while not is_finish_terrorist:  # 首先探索恐怖分子区域，首先还没有探索完成
            ...
            if room == "terrorist_room" or is_start_terrorist:  # 进入恐怖分子区域
                if not is_start_terrorist:  # 是否是首次进入恐怖分子区域
                    print("We enter the terrorist_room")
                ...
                if rospy.get_param('take_photo', 0) == 1:  # 从参数服务器获取参数，默认为0，该参数由导航提供，现在是否拍照
                    ...
                    if result != 0: #如果结果不为0
                        rospy.set_param("terrorist", result)  # 给控制端反馈
                        voice(self.classes["classes"][6-result])  # 语音播报恐怖分子数量
        ...
        while not is_finish_rescue:  # 再探测救援物品区域
            room = self.which_room()
            if room == "rescue_room" or is_start_rescue:  # 进入救援物品区域
                if not is_start_rescue:  # 首次进入救援物品房间
                    print("We enter the rescue room")
...
if __name__ == "__main__":  # 下面是真正运行的代码
    ...
    i = 0
    while not rospy.is_shutdowm():  # 循环等待启动识别
        if i > 30:
            break
        i += 1
        print(f"xunhuan_{i}")
