...
def start_condition_judge_5(nav):
    ...
    if nav.location == a_bridge_first:
        ...
        # nav.client_teb.update_configuration({'global_plan_overwrite_orientation': False})
        # nav.client_gp.update_configuration({'orientation_mode': 1}) 
        nav.client_teb.update_configuration({'xy_goal_tolerance' : 0.05})
        nav.client_teb.update_configuration({'yaw_goal_tolerance' : 0.1})
        # nav.client_teb.update_configuration({'weight_optimaltime' : 15})
        ...
        nav.client_teb.update_configuration({'xy_goal_tolerance' : 0.20})      
...
def insertG(nav):
    if nav.location == a_bridge_first:
        ...
        nav.locations['m_left_down'] = Pose(Point(0.920, -2.722, 0.000), Quaternion(0.000, 0.000, 0.874, 0.486))
        ...
        # nav.locations['G2'] = Pose(Point(0.509, -2.895, 0.000), Quaternion(0.000, 0.000, -0.674, 0.739))
        nav.locations['G1'] = Pose(Point(1.009, -3.395, 0.000), Quaternion(0.000, 0.000, 0.439, 0.898))
        nav.locations['G2'] = Pose(Point(1.264, -3.893, 0.000), Quaternion(0.000, 0.000, -0.674, 0.739))
...
def combined(nav):
    if nav.location == 'G1':
        ...
        for board in boards:
            # 障碍板是否在中央点附近
            point_in_convex_polygon(board.position, boardnav.get_central_square().flatten().tolist())
        ...
        # edge_angle_list = list(angle_list)
        for board in boards:
