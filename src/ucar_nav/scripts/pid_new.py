...
def odom_callback(msg):
    ...
    # 0515
    if msg.position.y >-0.5 and msg.position.x >1 and rospy.get_param('nongoal_flag') ==2:  # x坐标小于这个值时，切换到多点导航，同时需要给全局地图留出更新时间 
        rospy.set_param('nongoal_flag', 1)
        rospy.loginfo("pid_trace_end")
    if rospy.get_param('nongoal_flag') == 1 :
        if nearby_2d((msg.position.x,msg.position.y),end_point,0.5):   
    ...
    #     Twist_Pub(pub_cmd, 1, xspeed, zspeed)
        ...
        if msg.position.x > -0.55:  # x坐标小于这个值时，切换到多点导航，同时需要给全局地图留出更新时间 -0.453
            rospy.set_param('nongoal_flag', 5)
        yspeed = pidy2.pidCalY(( msg.position.y))
        Twist_Pub(pub_cmd, 0.6,  yspeed, 0)    
        rospy.loginfo(yspeed)
...
if __name__ == "__main__":
    ...
    pidy2= pidControlerYSpeed(y_bridge_s,5,0,0.5) # 第二次上桥
