...
int move_flag=0;
double yaw_all = 0;
sensor_msgs::Imu imu_data_init;
sensor_msgs::Imu imu_data;

namespace ucarController
{
baseBringup::baseBringup() :x_(0), y_(0), th_(0) // 原本x_和y_都是0 @cxy
{
  ...
  // move_sub_   = nh_.subscribe<std_msgs::Int32>("move_start", 1, &baseBringup::moveCallback, this);
...
void baseBringup::processIMU(uint8_t head_type)
{
  ...
  if (head_type == TYPE_AHRS)
  {
    ...
    // imu_pub_.publish(imu_data);
    // ROS_INFO("----------------------已经更正完yaw--------------------");
    // double yaw3=tf::getYaw(imu_data.orientation);
    // if(imu_flag<11){
    //   yaw_all = yaw_all + yaw3;
    //   yaw_all = yaw_all / 2;
    //   imu_flag+=1;
    // }
    // else{
...
// void baseBringup::moveCallback(const std_msgs::Int32::ConstPtr& msg)
// {
//   boost::unique_lock<boost::recursive_mutex> lock(Control_mutex_); 
//   move_flag = 1;
//   ROS_WARN("QAQ_QAQ");
...
void baseBringup::processOdometry(){
  ...
  // th_ += delta_th;
  /*******************/
  // rospy::param::get("move_flag", move_flag);
  double yaw3=tf::getYaw(imu_data.orientation);
  if(imu_flag<25){
    yaw_all = yaw_all + yaw3;
    imu_flag+=1;
    if(imu_flag>24){
      yaw_all = yaw_all / imu_flag;
    }
    ROS_INFO("__________________[%f]______________", yaw_all);
  }
  // double yaw2=tf::getYaw(imu_data.orientation);
  double yaw= yaw3 - yaw_all;
  if(yaw > 180){
    yaw = yaw - 360;
  }else if(yaw < -180){
    yaw = yaw + 360;
  ...
  th_ = yaw;
  /*******************/
  // double yaw1, yaw2, yaw;
  // if(move_flag == 1)
  // {
  //   yaw1=tf::getYaw(imu_data_init.orientation);
  //   yaw2=tf::getYaw(imu_data.orientation);
  // }
  // else
  // {
  //   yaw2=tf::getYaw(imu_data.orientation)-tf::getYaw(imu_data_init.orientation);
  //   yaw1=yaw2;
  //   if(std::isnan(yaw2))
  //   {
  ...
  odom_tmp.twist.twist.angular.z = imu_data.angular_velocity.z;
