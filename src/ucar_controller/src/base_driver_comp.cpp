...
{
baseBringup::baseBringup() :x_(0), y_(0), th_(0) //参数赋值
{
  ...
  wake_sub_    = nh_.subscribe<std_msgs::Int32>("/mic/awake/angle", 1, &baseBringup::wakeCallback, this);
...
void baseBringup::wakeCallback(const std_msgs::Int32::ConstPtr& msg)
{
  boost::unique_lock<boost::recursive_mutex> lock(Control_mutex_); 
  wake_flag = 1;
  lock.unlock();
...
void baseBringup::processOdometry(){
  ...
  double Vyaw = imu_data.angular_velocity.z;
  ...
  if (wake_flag == 0)
  {
    yaw1 = tf::getYaw(imu_data.orientation) - tf::getYaw(imu_data1.orientation);
    yaw2 = yaw1;
    if (std::isnan(yaw1))
      ROS_WARN("Yaw from base_driver is NAN!!!");
  }
  else
  {
    yaw2 = tf::getYaw(imu_data.orientation) - tf::getYaw(imu_data1.orientation);
  }
  //printf("yaw1:%f\nyaw2%f\n", yaw1, yaw2);
  yaw = yaw2 - yaw1;
  /*20220720-beaver*/
  ...
  //chen
  /*static double yaw_w = 0; 
  if (wake_flag)
  {  
    yaw_w += imu_data.angular_velocity.z * 1.0 / rate_;
  }
  th_ = yaw_w;*/
  ...
  //geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(yaw_w);//chen
  ...
  odom_tmp.twist.twist.angular.z = Vyaw;
  if (!Vx||!Vy||!Vth){
  ...
  if(provide_odom_tf_)
  {
    ...
    if (std::isnan(odom_trans.transform.translation.x) || std::isnan(odom_trans.transform.translation.y))
      ROS_WARN("Odom from base_driver has NAN!!!");
...
bool baseBringup::getMileage(){ //文件信息读取
  std::fstream fin;
  std::fstream fin_b;
  std::string str_in;
  std::string str_in_b;
